...
class BiasGene:
    ...
    def gene_distance(self, other):
        return abs(self.value - other.value)*compatibility_bias_coefficient
...
class WeightGene:
    ...
    def gene_distance(self, other):
        return abs(self.value - other.value)*compatibility_weight_coefficient
...
class NeuralNetworkModel:
    ...
    def noise_output(self):
        ...
        return numpy.random.uniform(-1, 1, size=ant_output_neruons)
    ...
    for g in range(generations):
        ...
        if g != generations - 1:
            ...
            if parent_selection_weighted_choises:
                ...
                normalize_fitness_performance = [round(abs(fitness - parent_fitnesses[-1])) / parent_fitnesses_range for
                                                 fitness in
                                                 parent_fitnesses]
            ...
            for i in range(ant_children):
                ...
                # Parent selection
                if parent_selection_weighted_choises:
                    parent1, parent2 = numpy.random.choice(potential_parents, size=2, p=reproduction_probability,
                                                           replace=False)
                else:
