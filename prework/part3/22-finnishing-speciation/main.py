...
from speciation import GenomeDistanceCashe, Species_set


def firstNitems(dictionary: dict, n: int):
    ant_created: int = 0
    new_dict: dict = {}
...
if __name__ == '__main__':
    ...
    config = Config()
    species_set = Species_set(config=config)
    ## Init population
    ...
    for g in range(config.generations):
        ...
        if g != config.generations - 1:
            ...
            # population = {k: v for k, v in sorted(population.items(), key=lambda item: item[1].fitness)}
            population = dict(sorted(population.items(), key=lambda item: item[1].fitness))
            ...
            survivers = firstNitems(population, ant_survivors)
            # survived = 0
            # for k, v in population.items(): # already sorted
            #     survivers[k]=v
            #     survived +=1
            #     if survived == ant_survivors:
            #         break
            # survivers = dict(sorted(population.items(), key=lambda item: item[1].fitness)) # 3.7 + preserves order
            ...
            potential_parents = list(firstNitems(dictionary=population, n=ant_parents).values())
            ...
            # population = survivers | children # 3.9
            population = {**survivers, **children}

            # Speciate the population
            species_set.speciate_population(population=population, generation=g)
