...
class Species_set:
    ...
    species_dict: dict = field(default_factory=dict, metadata={int:Species})
    genome_to_specie: dict = field(default_factory=dict, metadata={str: Species})
    genomeDistanceCashe: GenomeDistanceCashe = GenomeDistanceCashe()

    def speciate_population(self, population: dict, generation: int):
        # Find the best representatives for each existing species.
        unspeciated = {individual.id for individual in population.values()}
        new_representatives = {}
        new_members = {}
        ...
        for s_id, specie in self.species_dict.items():
            candidates = []
            for gid in unspeciated:
                g = population[gid]
                d = self.genomeDistanceCashe[specie.representative, g]
        ...
        while unspeciated:
            ...
            for sid, rid in new_representatives.items():
                rep = population[rid]
                d = self.get_genome_distance(genome1=rep.genome, genome2=individ.genome)
                if d < self.config.compatibility_threshold:
                    candidates.append((d, sid))

            if candidates:
                ignored_sdist, sid = min(candidates, key=lambda x: x[0])
                new_members[sid].append(gi)
            else:
                # No species is similar enough, create a new species, using
                # this genome as its representative.
                species_id = next(self.indexer)
                new_representatives[species_id] = individ_id
                new_members[species_id] = [individ_id]

        # Update species collection based on new speciation.
        self.genome_to_species = {}
        for sid, rid in new_representatives.items():
            s = self.species_dict.get(sid)
            if s is None:
                s = Species(key=sid, created=generation)
                self.species_dict[sid] = s
        ...
        print(self.genomeDistanceCashe.distances)
        print("speciation hits: {}, misses: {}".format(self.genomeDistanceCashe.hits,self.genomeDistanceCashe.misses))
        # gdmean = mean(self.genomeDistanceCashe.distances)
        # gdstdev = stdev(itervalues(distances.distances))
        # self.reporters.info(
        #     'Mean genetic distance {0:.3f}, standard deviation {1:.3f}'.format(gdmean, gdstdev))

    def get_genome_distance(self,genome1, genome2):
        ...
        for chromosome_key, genome1_chromsome in vars(genome1).items():
            ...
            chromosome_distances[chromosome_key] = self.genetic_chromosome_distance(genome1_chromsome, genome2_chromsome)
    ...
    def genetic_chromosome_distance(self,genome1_chromsome, genome2_chromsome):
        # Compute node gene distance component.
        disjoint_genes = 0
        ant_homologous_genes = 0
        total_distance_from_matching_genes = 0
        chromosome1_keys = genome1_chromsome.keys()
        chromosome2_keys = genome2_chromsome.keys()
        ...
        for k1 in chromosome1_keys:
            if k1 not in chromosome2_keys:
                ...
                total_distance_from_matching_genes += genome1_chromsome[k1].gene_distance(other=genome2_chromsome[k1], config = self.config)
        ...
        distance_from_disjoin_genes = (disjoint_genes * self.config.gene_compatibility_disjoint_coefficient) / max_genes
        average_homologous_gene_distance = 0 if ant_homologous_genes==0 else total_distance_from_matching_genes / ant_homologous_genes
